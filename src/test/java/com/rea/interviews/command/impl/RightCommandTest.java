...
public class RightCommandTest extends BaseTest {

	Robot robot = null;
	Command<Robot> command = new RightCommand();
	InvocationContext context = new InvocationContext("RIGHT");
	...
	public void testThatExecuteDoesNotChangeYCoordinate() throws Exception {
		Robot invocation = command.execute(robot, context);
		assertThat(invocation.getPosition().getY(), is(equalTo(1)));
	}
	...
	public void testThatExecuteMoveChangesFaceAndXCoordinate() throws Exception {
		...
		assertThat(bot.getPosition().getX(), is(equalTo(1)));
	}
